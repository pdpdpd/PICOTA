...
ALL_OTlist_dir = '/home/yp11/Desktop/genomes/1000genome/ftp.1000genomes.ebi.ac.uk/vol1/ftp/release/SNPMNPINDEL/COSMID_combined'
...
def tagscan_replace(cosmid_dir, input_dir, output_dir, ind, extended_seq, mm):
    # prepare gRNA sequence
    # ind = HG01556_1
    # mm only
    ...
    index_dir = input_dir + '/' + ind + '_idx.txt'
    target_dir = input_dir + '/' + ind + '_idx.bin'
    qry = cosmid_dir + ' -a ' + index_dir + ' -i ' + target_dir + ' -m ' + str(mm) + ' ' + extended_seq
    print(qry + ' > ' + output_dir)
    os.system(qry + ' > ' + output_dir)
    return
...
def main():
    ...
    #
    for ind_name in individual_arr:
        for hap_number in hap_arr:
            input_dir = ALL_folder_dir + '/' + 'hap' + str(hap_number) + '_' + ind_name + '_all'
            ind_hap = ind_name + '_' + str(hap_number)
            for gRNA in gRNA_arr:
                screen_output = ALL_OTlist_dir + '/' + ind_hap + gRNA + '.txt'
                cosmid_output = ALL_OTlist_dir + '/' + ind_hap + gRNA + 'cosmid.csv'
                filtered_output = ALL_OTlist_dir + '/' + ind_hap + gRNA + 'filtered.csv'
                final_output = ALL_OTlist_dir + '/' + ind_hap + gRNA + 'final.csv'
                temp_dir = ALL_OTlist_dir + '/temp.txt'
                ...
                df_2.to_csv(cosmid_output, index=False, header=True)
                ...
                #
                #     tagscan_replace(cosmid_dir, input_dir, temp_dir, ind_hap, extended_qry, 0)
                #
                ...
                # df_2 = df_2.sort_values(by=['COSMID'], ascending=False)
                # df_2.drop_duplicates(subset=['temp_chr', 'temp_origin_start', 'extended_seq'], keep='first',
                #                      inplace=True)
                ...
                #     #              + row.extended_seq
                #     crista_qry = 'python ' + crista_dir + '/CRISTA_yp.py -s ' + 'G' + gRNA + ' -d ' \
                #                  + row.extended_seq
                #     print(crista_qry)
